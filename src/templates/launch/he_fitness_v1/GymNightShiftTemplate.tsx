...
// ponytail: the 24h clock rail is the signature of the "Night Shift" design â€”
...
const CLOCK: { hr: string; blk: string; hot?: boolean }[] = [
  { hr: "00:00", blk: "Open floor Â· quiet hours begin" },
  { hr: "01:00", blk: "Night shift strength block", hot: true },
  { hr: "02:00", blk: "Open floor Â· sled lanes free" },
  { hr: "03:00", blk: "Open floor Â· coach form checks" },
  { hr: "04:00", blk: "Early crew conditioning" },
  { hr: "05:00", blk: "First light lift Â· coached", hot: true },
  { hr: "06:00", blk: "Pre-work strength block" },
  { hr: "07:00", blk: "Open floor Â· busiest morning hour" },
  { hr: "08:00", blk: "Mobility and recovery room" },
  { hr: "09:00", blk: "Open floor" },
  { hr: "10:00", blk: "Small group strength Â· 6 spots" },
  { hr: "11:00", blk: "Open floor" },
  { hr: "12:00", blk: "Lunch break intervals Â· 45 min", hot: true },
  { hr: "13:00", blk: "Open floor" },
  { hr: "14:00", blk: "Open floor Â· quietest daytime hour" },
  { hr: "15:00", blk: "Athlete prep Â· school age, coached" },
  { hr: "16:00", blk: "Open floor" },
  { hr: "17:00", blk: "After work strength block", hot: true },
  { hr: "18:00", blk: "Conditioning Â· sled and rower" },
  { hr: "19:00", blk: "Open floor" },
  { hr: "20:00", blk: "Small group strength Â· 6 spots" },
  { hr: "21:00", blk: "Open floor Â· floor resets" },
  { hr: "22:00", blk: "Closers club Â· service industry", hot: true },
  { hr: "23:00", blk: "Open floor Â· lights drop low" },
];

// ponytail: coaches carry name/shift/bio/photo â€” no content field models that shape
// today. Hardcoded as showcase design content. Upgrade path: a `coaches` content
// field so generated sites swap in real staff.
const COACHES = [
  {
    name: "Renata Diaz",
    shift: "Nights Â· 12am to 6am",
...
/** Colour the final word of a headline with the accent, Ã  la the original coral span. */
 ...
 * template â€” signature 24h clock rail, coaches, membership tiers, dark palette.
...
export function GymNightShiftTemplate({ content, theme, brandName }: LaunchTemplateProps) {
  ...
  const brand = brandName ?? c.seo?.title?.split("â€”")[0]?.trim() ?? "Your Gym";
  ...
  // Reorderable content sections keyed by SectionKey, rendered in theme order so
  // the editor's drag-to-reorder takes effect. Keys with no entry (about/why_us/
  // testimonials) render nothing, matching the bespoke design.
        ...
        <div className={`${WRAP} grid items-center gap-10 md:grid-cols-[7fr_5fr]`}>
          <div>
            <p className="mb-5 font-heading uppercase tracking-[0.28em] text-muted text-[0.95rem]">
              Doors never lock Â· 365 nights a year
            </p>
            <h1
  ...
  // ponytail: coaches is fixed showcase furniture (no section key). Pinned to
  // follow membership so the default look is unchanged; if pricing is reordered
  // coaches travels with it, and it hides when membership is hidden. Add a
  // `coaches` schema section if it ever needs independent reordering.
  const coaches = (
