...
const FAMILY = FAMILIES.trust_local;

/** Split-hero trust layout for local electricians (plan §13 launch template). */
export function ElectricianTrustTemplate({ content, theme, brandName }: LaunchTemplateProps) {
  ...
  const sectionMap = {
    hero: (
      <>
        <HeroSplit content={content.hero} theme={theme} eyebrow="Licensed Electrician" />
        <TrustStrip points={content.why_us.points.slice(0, 3)} />
      </>
    ),
    services: <ServicesCards content={content.services} theme={theme} />,
  ...
  return (
    <div className={templateFontClasses}>
      <SiteRoot theme={theme} family={FAMILY} templateId="electrician_trust_v1">
